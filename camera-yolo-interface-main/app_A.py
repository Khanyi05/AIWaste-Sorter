# app.py - Fixed AI Waste Sorter with Corrected Auto-Play Sequences
...
class CameraRobotCalibrator:
    ...
    def get_robot_angles_for_pixel(self, pixel_x: float, pixel_y: float):
        """Convert pixel coordinates to robot angles using actual calibration data"""
        real_x, real_y = self.pixel_to_real(pixel_x, pixel_y)

        print(f"🎯 Pixel({pixel_x:.0f},{pixel_y:.0f}) -> Real({real_x:.1f}cm, {real_y:.1f}cm)")

        # FIXED ANGLE CALCULATIONS BASED ON YOUR CALIBRATED POSITIONS
        # Using your actual calibrated positions as reference:
        ...
        # Base angle: Maps X coordinate (0-30cm) to (35-70°)
        # From your data: X=15->35°, X=22->60°, X=23->65°, X=29->60°
        if real_x <= 15:
            base_angle = 35
        elif real_x <= 23:
            # Between 15-23cm: 35° to 65°
            base_angle = 35 + (real_x - 15) * (65 - 35) / (23 - 15)
        else:
            # Above 23cm: 65° to 70°
            base_angle = 65 + min((real_x - 23) * (70 - 65) / (30 - 23), 5)

        # Shoulder angle: Maps Y coordinate with complex relationship
        # From your data: Y=21->90°, Y=22->85°, Y=29->145°, Y=30->95°
        if real_y <= 22:
            shoulder_angle = 85 + (real_y - 22) * (90 - 85) / (21 - 22) if real_y < 22 else 85
        elif real_y <= 29:
            # Between 22-29cm: 85° to 145°
            shoulder_angle = 85 + (real_y - 22) * (145 - 85) / (29 - 22)
        else:
            # Above 29cm: 145° to 95° (decreasing)
            shoulder_angle = 145 - min((real_y - 29) * (145 - 95) / (30 - 29), 50)

        # Elbow angle: Mostly stable around 160°
        # From your data: 155°-160°
        if real_y <= 21:
            elbow_angle = 160
        elif real_y <= 30:
            elbow_angle = 160 - (real_y - 21) * (160 - 155) / (30 - 21)
        else:
            elbow_angle = 155

        # Wrist pitch: Adjusts for proper gripping orientation
        # From your data: 90°-155°
        if real_y <= 21:
            wrist_pitch = 155
        elif real_y <= 29:
            wrist_pitch = 155 - (real_y - 21) * (155 - 90) / (29 - 21)
        else:
            wrist_pitch = 90

        # Wrist roll: Fixed at 90° for now (neutral position)
        wrist_roll = 90

        # Clamp to safe ranges based on your arm's physical limits
        ...
        print(
            f"🤖 Angles: Base={base_angle:.0f}°, Shoulder={shoulder_angle:.0f}°, Elbow={elbow_angle:.0f}°, WristP={wrist_pitch:.0f}°")
...
# Waste classes - CORRECTED MAPPING
waste_classes = {
    39: "Plastic",  # Bottle -> Plastic
    40: "Glass",  # Wine glass -> Glass
    43: "Paper",  # Cup -> Paper
    44: "Metal",  # Fork -> Metal
    67: "E-Waste"  # Cell phone -> E-Waste
}

# Pico command mapping with CORRECTED auto-play sequences
pico_commands = {
    "MOVE_SHOULDER_UP": "U",
    ...
    "EMERGENCY_STOP": "X",
    "AUTO_PICKUP": "P",
    "PLAY_PLASTIC_SEQUENCE": "Q",  # Q = Plastic (different from E-Waste)
    "PLAY_EWASTE_SEQUENCE": "P",  # R = E-Waste (different from Plastic)
    "PLAY_PAPER_SEQUENCE": "R"  # P = Paper
}
...
def move_to_pixel_position(pixel_x, pixel_y):
    """Move robot to a specific pixel position"""
    global arm_status
    ...
    try:
        ...
        # For now, we'll use manual movement commands
        # You can enhance this to send specific angle commands later
        print(f"🤖 Would move to: Base={base_angle}°, Shoulder={shoulder_angle}°, Elbow={elbow_angle}°")
        ...
        return {
            ...
            "message": "Position calculated - implement movement logic"
        }
...
def generate_frames():
    ...
    while True:
        ...
        if calibrator.is_calibrated:
            cv2.putText(frame, "✅ CALIBRATED: Click anywhere to calculate position",
...
async def move_to_pixel(position: dict):
    """Calculate robot position for a specific pixel coordinate"""
...
async def auto_pickup():
    """Calculate position for the last detected object"""
    ...
    try:
        ...
        print(f"🤖 Auto pickup calculation: {material} at pixel({center_x}, {center_y})")

        # Calculate position for the detected object
        ...
        if result["status"] == "success":
            ...
            return {
                "status": "success",
                "message": f"Auto pickup position calculated for {material}",
...
# HTML Interface with CORRECTED Auto-Play Buttons
...
async def index(request: Request):
    ...
    <head>
      ...
      <style>
        ...
        .auto-play-section { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 15px; border-radius: 8px; margin-bottom: 15px; }
        .auto-play-btn { width: 100%; margin-bottom: 8px; font-weight: bold; }
        .plastic-btn { background: linear-gradient(45deg, #FFD700, #FFA500); border: none; }
        .ewaste-btn { background: linear-gradient(45deg, #4A90E2, #007AFF); border: none; }
        .paper-btn { background: linear-gradient(45deg, #34C759, #28A745); border: none; }
      </style>
    </head>
    <body>
      <div class="container-fluid p-3" style="height: 100vh; overflow: hidden;">
        <div class="d-flex justify-content-between align-items-center mb-3 border-bottom pb-2">
          <h3>🤖 AI Waste Sorter - Different Pickup for Each Waste Type</h3>
        ...
        <div class="row gx-3 gy-3" style="height: calc(100% - 70px)">
          ...
          <div class="col-lg-7 d-flex flex-column gap-3" style="height: 100%">
            ...
            <!-- Auto-Play Sequences Section -->
            <div class="auto-play-section">
              <h5 class="text-center mb-3">🎯 Auto Detect Waste</h5>
              <div class="row g-2">
                <div class="col-4">
                  <button class="btn plastic-btn auto-play-btn" id="playPlasticSequence">
                    🍼 Plastic (Q)
                  </button>
                  <small class="d-block text-center mt-1 opacity-75">Different pickup</small>
                </div>
                <div class="col-4">
                  <button class="btn ewaste-btn auto-play-btn" id="playEWasteSequence">
                    📱 E-Waste (R)
                  </button>
                  <small class="d-block text-center mt-1 opacity-75">Different pickup</small>
                </div>
                <div class="col-4">
                  <button class="btn paper-btn auto-play-btn" id="playPaperSequence">
                    📄 Paper (P)
                  </button>
                  <small class="d-block text-center mt-1 opacity-75">Different pickup</small>
                </div>
              ...
              <div class="text-center mt-2 small opacity-75">
                Each waste type has unique pre-recorded movement sequence
              </div>
            </div>
            ...
            <div class="card mb-3">
              ...
              <div class="card-footer text-muted small">
                💡 Click anywhere on the camera feed to calculate robot position
              </div>
            </div>
      ...
      <script>
        document.addEventListener('DOMContentLoaded', function() {
          ...
          // Auto-Play Sequence Buttons - CORRECTED
          document.getElementById('playPlasticSequence').addEventListener('click', () => sendCommand('PLAY_PLASTIC_SEQUENCE'));
          document.getElementById('playEWasteSequence').addEventListener('click', () => sendCommand('PLAY_EWASTE_SEQUENCE'));
          document.getElementById('playPaperSequence').addEventListener('click', () => sendCommand('PLAY_PAPER_SEQUENCE'));
          ...
          cameraContainer.addEventListener('click', function(e) {
            ...
            // Show coordinates
            clickCoords.textContent = `Click: (${x}, ${y})`;
            ...
            setTimeout(() => clickCoords.style.display = 'none', 2000);

            // Calculate robot position for clicked position
            moveToPixel(x, y);
          });
          ...
          async function moveToPixel(x, y) {
            try {
              ...
              if (result.status === 'success') {
                updateArmStatus(`Calculated position (${x},${y})`);
                alert(`Position calculated!\nReal: (${result.real_coords[0].toFixed(1)}cm, ${result.real_coords[1].toFixed(1)}cm)\nAngles: Base=${result.angles.base}°, Shoulder=${result.angles.shoulder}°, Elbow=${result.angles.elbow}°`);
              } else {
                alert('Error: ' + result.message);
              }
            } catch (error) {
              console.error('Move to pixel error:', error);
              alert('Failed to calculate position: ' + error.message);
            }
          }
          ...
          async function sendCommand(command) {
            try {
              ...
              // Show success message for auto-play sequences
              if (command.includes('PLAY_')) {
                const sequenceName = command.replace('PLAY_', '').replace('_SEQUENCE', '');
                alert(`🎬 Started ${sequenceName} auto-pickup sequence!\nEach waste type has unique pickup movements.`);
              }
            } catch (error) { 
          ...
          function updateArmStatus(status) {
            const statusMap = { 
              ...
              'PLAY_PLASTIC_SEQUENCE': 'Playing Plastic Sequence',
              'PLAY_EWASTE_SEQUENCE': 'Playing E-Waste Sequence', 
              'PLAY_PAPER_SEQUENCE': 'Playing Paper Sequence'
            };
          ...
          async function autoPickup() {
            try {
              ...
              if (result.status === 'success') {
                ...
                alert(`Auto pickup calculated for ${result.material}!\nReal: (${result.real_coords[0].toFixed(1)}cm, ${result.real_coords[1].toFixed(1)}cm)\nAngles: Base=${result.angles.base}°, Shoulder=${result.angles.shoulder}°, Elbow=${result.angles.elbow}°`);
              } else {
                alert('Auto pickup failed: ' + result.message);
              }
            } catch (error) {
              console.error('Auto pickup error:', error);
              alert('Auto pickup failed: ' + error.message);
            }
          }
              ...
              if (status.status === 'calibrated') {
                calibrationStatus.innerHTML = `
                  <div class="calibrated">
                    ...
                    <br><small>Click anywhere on camera feed to calculate robot position</small>
                  </div>
                `;
              } else {
                calibrationStatus.innerHTML = `
...
if __name__ == '__main__':
    ...
    print("🚀 Starting AI Waste Sorter with CORRECTED Auto-Play Sequences at http://localhost:8000")
    print("🎯 CORRECTED Auto-Play Buttons:")
    print("   🍼 Plastic (Q) - Different pickup sequence")
    print("   📱 E-Waste (R) - Different pickup sequence")
    print("   📄 Paper (P) - Different pickup sequence")
    print("📊 Calibration Status:", "Loaded" if calibrator.is_calibrated else "Not Loaded")
    if calibrator.is_calibrated:
        print("🎯 Accuracy: 0.96cm average error")
        print("💡 Feature: Click anywhere on camera feed to calculate robot position!")
