# app.py - Complete AI Waste Sorter with Hardcoded Calibration
...
class CameraRobotCalibrator:
    ...
    def get_robot_angles_for_pixel(self, pixel_x: float, pixel_y: float):
        """Convert pixel coordinates to robot angles"""
        real_x, real_y = self.pixel_to_real(pixel_x, pixel_y)

        print(f"🎯 Pixel({pixel_x:.0f},{pixel_y:.0f}) -> Real({real_x:.1f}cm, {real_y:.1f}cm)")

        # Enhanced angle calculation based on real coordinates
        # Base angle: maps X coordinate (0-45cm) to (35-90°)
        base_angle = 35 + (real_x / 45) * 55

        # Shoulder angle: maps Y coordinate and adjusts for reach
        if real_y <= 15:
        ...
        # Elbow angle: adjusts based on distance
        if real_y <= 20:
            elbow_angle = 160
        else:
            elbow_angle = 160 - min((real_y - 20) * 0.5, 15)

        # Wrist angles for proper orientation
...
if __name__ == '__main__':
    ...
    print("🚀 Starting AI Waste Sorter with HARDCODED Calibration at http://localhost:8000")
