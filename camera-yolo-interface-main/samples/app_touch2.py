# app.py - AI Waste Sorter with Automatic Movement to Clicked Positions
...
pico_commands = {
    ...
    "GRIP_CLOSE": "O",
    "EMERGENCY_STOP": "X",
    "AUTO_PICKUP": "P",  # New: Auto pickup command
    "MOVE_TO_ANGLES": "M"  # New: Move to specific angles command
}
...
def move_to_angles(base_angle, shoulder_angle, elbow_angle, wrist_pitch, wrist_roll):
    """Send specific angle commands to the Pico"""
    global current_angles

    if not pico_serial:
        print("⚠️ Pico not connected - simulating movement")
        current_angles = {
            "base": base_angle,
        ...
        return "SIMULATED_MOVEMENT"

    try:
        # Format: M,base,shoulder,elbow,wrist_pitch,wrist_roll
        angle_command = f"M,{base_angle},{shoulder_angle},{elbow_angle},{wrist_pitch},{wrist_roll}"
        response = send_pico_command(angle_command, delay=2.0)  # Longer delay for movement

        # Update current angles if movement was successful
        if "MOVED" in response or "OK" in response:
            current_angles = {
                "base": base_angle,
                "shoulder": shoulder_angle,
                "elbow": elbow_angle,
        ...
        return f"ANGLE_MOVE_ERROR:{str(e)}"


def move_to_pixel_position(pixel_x, pixel_y):
    """Move robot to a specific pixel position AUTOMATICALLY"""
    ...
    try:
        ...
        movement_result = move_to_angles(base_angle, shoulder_angle, elbow_angle, wrist_pitch, wrist_roll)
        ...
        return {
            ...
            "message": f"Arm moving to position! Result: {movement_result}"
        }
...
def auto_pickup_sequence():
    """Complete auto pickup sequence: move to object, grip, and return"""
    ...
    try:
        ...
        print(f"🤖 Starting auto pickup sequence for {material} at ({center_x}, {center_y})")
        ...
        grip_result = send_cmd("GRIP_OPEN", delay=1.0)

        time.sleep(0.5)

        # Step 3: Close gripper to pick up object
        print("🤖 Closing gripper to pick up object...")
        grip_result = send_cmd("GRIP_CLOSE", delay=1.0)

        time.sleep(1)

        # Step 4: Move to home position (you can define this)
        print("🤖 Returning to home position...")
        home_result = move_to_angles(60, 90, 160, 90, 90)
...
async def api_move_to_angles(angles: dict):
    """Move to specific angles directly"""
    global arm_status

    try:
        base = angles.get("base", current_angles["base"])
        shoulder = angles.get("shoulder", current_angles["shoulder"])
        elbow = angles.get("elbow", current_angles["elbow"])
        wrist_pitch = angles.get("wrist_pitch", current_angles["wrist_pitch"])
        wrist_roll = angles.get("wrist_roll", current_angles["wrist_roll"])

        with arm_lock:
            arm_status = f"moving_to_angles_b{base}_s{shoulder}"

        result = move_to_angles(base, shoulder, elbow, wrist_pitch, wrist_roll)

        return {
            "status": "success",
            "message": f"Moved to angles: Base={base}°, Shoulder={shoulder}°",
...
# HTML Interface (updated JavaScript for automatic movement)
@app.get("/", response_class=HTMLResponse)
async def index(request: Request):
    # [Keep the same HTML structure but update the JavaScript part]
    return """
    <!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    <body>
      ...
      <script>
        document.addEventListener('DOMContentLoaded', function() {
          ...
          // Click-to-move functionality - NOW AUTOMATIC MOVEMENT!
          ...
          async function moveToPixel(x, y) {
            try {
              ...
              if (result.status === 'success') {
                ...
                showNotification(`Arm moved to position! Real: (${result.real_coords[0].toFixed(1)}cm, ${result.real_coords[1].toFixed(1)}cm)`, 'success');
...
if __name__ == '__main__':
    ...
    print("🚀 Starting AI Waste Sorter with AUTOMATIC MOVEMENT at http://localhost:8000")
