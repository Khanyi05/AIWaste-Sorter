# app.py - COMPLETE FIXED VERSION WITH WORKING MOVEMENT
...
# Updated Camera-Robot Calibrator with PRECISE (342,418) -> (29,29) mapping
class CameraRobotCalibrator:
    def __init__(self):
        ...
        # UPDATED: PRECISE CALIBRATION WITH (342,418) -> (29,29) AS EXACT MATCH
        self._load_precise_calibration()

    def _load_precise_calibration(self):
        """Load precise calibration with (342,418) as the exact (29,29) position"""
        print("📂 Loading PRECISE calibration data...")

        # Your original calibration points, but with (342,418) as the exact (29,29) match
        self.calibration_points = [
            (342, 418, 29.0, 29.0),  # EXACT CURRENT POSITION - ARM IS HERE!
            (321, 256, 15.0, 15.0),
            (206, 328, 15.0, 30.0),
            (128, 239, 0.0, 30.0),
            (515, 273, 30.0, 0.0),
        ...
        if self.transform_matrix is not None:
            ...
            print(f"✅ PRECISE Calibration loaded: {len(self.calibration_points)} points")
            print(f"🎯 EXACT MATCH: Pixel(342,418) -> Real(29cm,29cm)")
    ...
    def get_robot_angles_for_pixel(self, pixel_x: float, pixel_y: float):
        """Convert pixel coordinates to robot angles with PRECISE mapping"""
        real_x, real_y = self.pixel_to_real(pixel_x, pixel_y)

        print(f"🎯 Pixel({pixel_x:.0f},{pixel_y:.0f}) -> Real({real_x:.1f}cm, {real_y:.1f}cm)")

        # PRECISE MAPPING BASED ON YOUR CALIBRATED POSITIONS
        calibrated_positions = {
            (29, 29): (60, 145, 160, 90),  # Base, Shoulder, Elbow, Wrist - CURRENT POSITION
            (15, 15): (60, 85, 160, 155),
            (15, 30): (35, 95, 155, 150),
            (0, 30): (30, 100, 155, 150),
            (30, 0): (65, 80, 160, 155),
        ...
        base_angle, shoulder_angle, elbow_angle, wrist_pitch = 60, 145, 160, 90  # Default to current
        ...
        if min_distance < 5:  # Within 5cm of a calibrated position
            print(f"🎯 USING EXACT CALIBRATED ANGLES for position")
        else:
            ...
            # Simple interpolation between calibrated positions
            if real_x <= 22:
...
# Initialize calibrator with PRECISE mapping
...
current_angles = {"base": 60, "shoulder": 145, "elbow": 160, "wrist_pitch": 90,
                  "wrist_roll": 90}  # Updated to current position
...
def move_to_angles_smart(base_target, shoulder_target, elbow_target, wrist_pitch_target, wrist_roll_target):
    ...
    print(
        f"📍 From current: Base={current_angles['base']}°, Shoulder={current_angles['shoulder']}°, Elbow={current_angles['elbow']}°")
    ...
    print(f"📊 Differences: Base={base_diff}°, Shoulder={shoulder_diff}°, Elbow={elbow_diff}°")

    # Check if we're already at the target position
    if (abs(base_diff) < 5 and abs(shoulder_diff) < 5 and abs(elbow_diff) < 5):
        print("📍 Already at target position - no movement needed")
        return "ALREADY_AT_POSITION"
    ...
    if abs(base_diff) > 5:  # Only move if significant difference
        if base_diff > 0:
            movements.extend(["MOVE_BASE_RIGHT"] * int(abs(base_diff) // 5))
        else:
            movements.extend(["MOVE_BASE_LEFT"] * int(abs(base_diff) // 5))

    # Shoulder movement
    if abs(shoulder_diff) > 5:
        if shoulder_diff > 0:
            movements.extend(["MOVE_SHOULDER_UP"] * int(abs(shoulder_diff) // 5))
        else:
            movements.extend(["MOVE_SHOULDER_DOWN"] * int(abs(shoulder_diff) // 5))

    # Elbow movement
    if abs(elbow_diff) > 5:
        if elbow_diff > 0:
            movements.extend(["MOVE_ELBOW_UP"] * int(abs(elbow_diff) // 5))
        else:
            movements.extend(["MOVE_ELBOW_DOWN"] * int(abs(elbow_diff) // 5))

    print(f"🔄 Movement sequence: {len(movements)} commands: {movements}")
...
def generate_frames():
    ...
    while True:
        ...
        # Add calibration info overlay with CURRENT POSITION marker
        if calibrator.is_calibrated:
            # Mark the current arm position (342,418) with a special indicator
            cv2.circle(frame, (342, 418), 8, (0, 255, 255), -1)  # Yellow circle
            cv2.putText(frame, "CURRENT ARM POSITION", (250, 410),
...
async def get_calibration_status():
    ...
    if calibrator.is_calibrated:
        return {
            ...
            "ready": True,
            "current_position": "Pixel(342,418) -> Real(29cm,29cm)"
        }
    else:
        return {
...
# HTML Interface (COMPLETE VERSION)
...
async def index(request: Request):
    ...
    <head>
      ...
      <style>
        ...
        .current-position { background-color: #fff3cd; border: 2px solid #ffc107; padding: 10px; border-radius: 5px; margin-bottom: 15px; }
    ...
    <body>
      <div class="container-fluid p-3" style="height: 100vh; overflow: hidden;">
        ...
        <div class="current-position">
          <strong>📍 CURRENT ARM POSITION:</strong> Pixel(342,418) → Real(29cm,29cm) 
          <br><small>Look for the <span style="color: #ffc107">yellow circle</span> on camera feed - arm is currently here!</small>
        </div>

        <div class="row gx-3 gy-3" style="height: calc(100% - 120px)">
          <!-- Left Column -->
          <div class="col-lg-7 d-flex flex-column gap-3" style="height: 100%">
            ...
            <div class="card mb-3">
              ...
              <div class="card-footer text-muted small">
                ...
                <br>🎯 <strong>Yellow circle</strong> shows where the arm is currently positioned.
              </div>
            </div>
              ...
              if (status.status === 'calibrated') {
                calibrationStatus.innerHTML = `
                  <div class="calibrated">
                    <strong>✅ PRECISE CALIBRATION LOADED</strong> - ${status.points} points, ${status.avg_error} avg error
                    <br><small><strong>EXACT MATCH:</strong> ${status.current_position}</small>
...
if __name__ == '__main__':
    ...
    print("🚀 Starting AI Waste Sorter with PRECISE AUTOMATIC MOVEMENT at http://localhost:8000")
    print("📊 Calibration Status:", "Loaded" if calibrator.is_calibrated else "Not Loaded")
    if calibrator.is_calibrated:
        print("🎯 EXACT MATCH: Pixel(342,418) -> Real(29cm,29cm)")
        print("💡 FEATURE: Click anywhere on camera feed to automatically move the arm!")
        print("📍 CURRENT ARM POSITION: Marked with yellow circle on camera feed")
